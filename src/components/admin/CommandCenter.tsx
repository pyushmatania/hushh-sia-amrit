...
export default function CommandCenter({ onNavigate }: { onNavigate?: (page: AdminPage) => void }) {
  ...
  const miniStatCards = [
    ...
    { label: "Avg Rating", value: stats.avgRating, icon: Star, color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-500/10", border: "border-amber-100/80 dark:border-amber-500/20" },
  ...
  const widgets = useMemo(() => [
      ...
      render: () => (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {smartCards.map((card, i) => (
            <motion.div key={i} whileHover={{ scale: 1.02 }} className={`${card.bg} rounded-2xl p-4 flex items-center gap-3 border ${card.border} cursor-pointer group`}>
              <div className="w-10 h-10 rounded-xl bg-white/70 dark:bg-white/10 flex items-center justify-center shrink-0 shadow-sm">
                <card.icon size={18} className={card.color} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-zinc-700 dark:text-zinc-200 font-medium leading-snug">{card.text}</p>
                <p className="text-[10px] text-zinc-400 mt-0.5 flex items-center gap-1"><Sparkles size={9} /> AI Insight</p>
              </div>
      ...
      render: () => (
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2"><Zap size={14} className="text-amber-500" />Quick Shortcuts</h3>
            <span className="text-[10px] text-zinc-400">{quickShortcuts.length} actions</span>
          </div>
          <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
            {quickShortcuts.map(action => (
              <motion.button key={action.label} whileHover={{ y: -3, scale: 1.03 }} whileTap={{ scale: 0.95 }} onClick={() => onNavigate?.(action.page)} className={`${action.bg} rounded-xl p-3 flex flex-col items-center gap-1.5 border border-zinc-100/50 dark:border-zinc-800/50 hover:shadow-md transition group`}>
    ...
    },
    {
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center"><Calendar size={14} className="text-indigo-600" /></div>
              Today's Schedule
              {todaySchedule.length > 0 && <span className="px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 text-[10px] font-bold">{todaySchedule.length}</span>}
            </h3>
            <button onClick={() => onNavigate?.("calendar")} className="text-[11px] text-indigo-500 font-medium flex items-center gap-0.5 hover:underline">Full calendar <ChevronRight size={12} /></button>
          </div>
          {todaySchedule.length === 0 ? (
            <div className="text-center py-6"><Calendar size={24} className="mx-auto text-zinc-300 dark:text-zinc-600 mb-2" /><p className="text-xs text-zinc-400">No bookings scheduled for today</p></div>
          ) : (
            <div className="space-y-2">
              {todaySchedule.map((slot, i) => {
                const SlotIcon = getSlotIcon(slot.slot);
                return (
                  <div key={slot.id + i} className="flex items-center gap-3 p-3 rounded-xl bg-zinc-50/80 dark:bg-zinc-800/50 border border-zinc-100/50 dark:border-zinc-700/50 group hover:border-indigo-200 dark:hover:border-indigo-500/30 transition">
                    <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-100 to-violet-100 dark:from-indigo-500/20 dark:to-violet-500/20 flex items-center justify-center shadow-sm"><SlotIcon size={16} className="text-indigo-600 dark:text-indigo-400" /></div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-zinc-700 dark:text-zinc-200 truncate">{slot.propertyName}</p>
                      <p className="text-[10px] text-zinc-400 flex items-center gap-1.5"><Clock size={8} /> {slot.slot} · <Users size={8} /> {slot.guests} guests</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xs font-bold text-zinc-700 dark:text-zinc-200 tabular-nums">₹{slot.total.toLocaleString()}</p>
                      <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${slot.status === "confirmed" ? "bg-emerald-100 dark:bg-emerald-500/15 text-emerald-600" : "bg-amber-100 dark:bg-amber-500/15 text-amber-600"}`}>{slot.status === "confirmed" ? "✓ Confirmed" : "⏳ Pending"}</span>
                    </div>
                  </div>
                );
              })}
    ...
    },
    {
      ...
      label: "AI Search",
      render: () => (
        <NeuralSearchWidget title="Ask AI" subtitle="Business intelligence at your fingertips" placeholder="Ask about revenue, guests, trends..." examples={commandCenterExamples} onSearch={handleCommandCenterSearch} />
      ),
    },
    {
      id: "live-orders",
      ...
      render: () => (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2">
                <div className="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center"><TrendingUp size={14} className="text-emerald-600" /></div>Top Properties
              </h3>
              <button onClick={() => onNavigate?.("analytics")} className="text-[11px] text-indigo-500 font-medium flex items-center gap-0.5 hover:underline">Details <ChevronRight size={12} /></button>
            </div>
                  ...
                  return (
                    <div key={prop.name} className="space-y-1">
                      ...
                      <div className="h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${(prop.revenue / maxRev) * 100}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className="h-full rounded-full bg-gradient-to-r from-indigo-400 to-violet-400" />
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
            <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2 mb-4">
              <div className="w-7 h-7 rounded-lg bg-violet-50 dark:bg-violet-500/10 flex items-center justify-center"><PieChart size={14} className="text-violet-600" /></div>Category Mix
            </h3>
    ...
    },
    {
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center"><MessageSquare size={14} className="text-amber-600" /></div>Recent Reviews
          </h3>
            ...
            <div className="space-y-2.5">
              {recentReviews.map((review) => (
                <div key={review.id} className="flex items-start gap-3 p-3 rounded-xl bg-zinc-50/80 dark:bg-zinc-800/50 border border-zinc-100/50 dark:border-zinc-700/50">
                  <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-100 to-yellow-100 dark:from-amber-500/20 dark:to-yellow-500/20 flex items-center justify-center shadow-sm shrink-0"><Star size={14} className="text-amber-600 fill-amber-500" /></div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-0.5 mb-1">
                      {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={10} className={i < review.rating ? "text-amber-400 fill-amber-400" : "text-zinc-200 dark:text-zinc-700"} />)}
                    </div>
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2"><Activity size={16} className="text-indigo-500" />Weekly Performance</h3>
              <p className="text-xs text-zinc-400 mt-0.5">Booking volume by day</p>
            </div>
            <div className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-500/10"><TrendingUp size={12} className="text-indigo-500" /><span className="text-[10px] font-semibold text-indigo-600">+18%</span></div>
          </div>
          <div className="h-[200px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={weeklyData} barCategoryGap="25%">
                ...
                <Tooltip contentStyle={{ background: "#fff", border: "1px solid #e4e4e7", borderRadius: 16, fontSize: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.08)", padding: "10px 14px" }} cursor={{ fill: "rgba(99,102,241,0.04)", radius: 8 } as any} />
    ...
    },
    {
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center"><Shield size={14} className="text-emerald-600" /></div>System Health
          </h3>
    ...
    },
    {
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2"><IndianRupee size={16} className="text-emerald-500" />Revenue Trend</h3>
              <p className="text-xs text-zinc-400 mt-0.5">Last 14 days performance</p>
            </div>
            <div className="text-right">
              <p className="text-lg font-bold text-zinc-800 dark:text-zinc-100 tabular-nums">₹{mockChartData.reduce((s, d) => s + d.revenue, 0).toLocaleString("en-IN")}</p>
          ...
          <div className="h-[220px] md:h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={mockChartData}>
                ...
                <Tooltip contentStyle={{ background: "#fff", border: "1px solid #e4e4e7", borderRadius: 16, fontSize: 12, boxShadow: "0 8px 24px rgba(0,0,0,0.08)", padding: "10px 14px" }} />
    ...
    },
    {
      ...
      render: () => (
        <div className="rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm border border-zinc-100/80 dark:border-zinc-800/80 p-5">
          <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-2 mb-4">
            <div className="w-7 h-7 rounded-lg bg-violet-50 dark:bg-violet-500/10 flex items-center justify-center"><Target size={14} className="text-violet-600" /></div>Conversion Funnel
          </h3>
          <div className="space-y-2">
            {[
              ...
              <div key={step.stage} className="flex items-center gap-3">
                ...
                <div className="flex-1 h-6 bg-zinc-100 dark:bg-zinc-800 rounded-lg overflow-hidden relative">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${step.pct}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className={`h-full rounded-lg bg-gradient-to-r ${step.color} flex items-center justify-end pr-2`}>
        ...
        {editMode && (
          ...
          >
            ...
            <div className="flex-1">
              <p className="text-xs font-semibold text-foreground">Customization Mode</p>
              <p className="text-[10px] text-muted-foreground">Drag widgets to rearrange your dashboard layout</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Main Stat Cards — always on top, not draggable */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {statCards.map((card) => (
          ...
          >
            ...
            <p className="relative text-2xl font-bold text-zinc-800 dark:text-zinc-100 tabular-nums"><AnimatedCounter value={card.value} prefix={card.prefix} /></p>
            <p className="relative text-[11px] text-zinc-500 mt-0.5 font-medium">{card.label}</p>
          </motion.div>
        ))}
      </div>

      {/* Mini Stats — always on top */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {miniStatCards.map((card) => (
          <motion.div key={card.label} variants={fadeUp} className={`${card.bg} rounded-xl p-3 border ${card.border} flex items-center gap-2.5`}>
            <div className="w-8 h-8 rounded-lg bg-white/70 dark:bg-white/10 flex items-center justify-center shadow-sm shrink-0"><card.icon size={14} className={card.color} /></div>
            <div className="min-w-0">
              <p className="text-lg font-bold text-zinc-800 dark:text-zinc-100 tabular-nums leading-none flex items-center gap-1">
                {card.value}
                {card.alert && <motion.span animate={{ scale: [1, 1.3, 1] }} transition={{ repeat: Infinity, duration: 1.5 }} className="w-2 h-2 rounded-full bg-rose-500 inline-block" />}
              </p>
