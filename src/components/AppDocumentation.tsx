...
export const changeLog = [
  ...
  { version: "1.37", phase: "Credits & Colophon", items: ["Credits/Colophon tab in Easter egg docs — behind-the-scenes engineering stats", "Dev effort estimation: solo dev days, team scenarios, months breakdown", "Code breakdown by area: Frontend UI, Business Logic, Admin, Backend, Styling, Infra", "Fun facts: coffee consumed, keystrokes typed, features per release", "PDF export updated with full Credits & Colophon section", "Build error fixes: edge functions TS errors, vite.config.ts rollup types"] },
  { version: "1.38", phase: "Emil Kowalski Motion System", items: ["Integrated Emil Kowalski animation skill set — 25+ motion presets in src/lib/animations.ts", "iOS-grade spring physics: SPRING_SNAPPY (stiffness 300, damping 30), SPRING_BOUNCY (stiffness 400, damping 15), SPRING_IOS_SHEET (stiffness 280, damping 28)", "Custom easings: EASE_IOS [0.32, 0.72, 0, 1], EASE_OUT_SMOOTH [0.22, 1, 0.36, 1]", "blurIn animation — elements transition from blur(10px) to blur(0px) with opacity + Y offset", "blurInScale — modal/popup entrance with combined blur + scale + opacity", "staggerItemScale — grid card entrances with sequential delay", "textRevealUp — hero heading character-by-character upward reveal", "listSlideIn — horizontal slide with blur trail for chat messages and notifications", "popIn / badgePop — elastic scale-up for badges, tags, notification dots", "modalScale — dialog entrance with spring-based scale + blur backdrop", "numberRoll — animated counter for prices, points, stats", "subtleFloat — perpetual idle float with brightness glow on hover", "heartPop — like button animation with overshoot spring", "magneticConfig — CTA buttons that follow cursor proximity", "SplashScreen: textRevealUp greeting + blurInScale logo entrance", "HomeScreen: whileInView blur-in reveals on section titles", "BottomNav: blur-in entrance + spring tap feedback (scale 0.88)", "PropertyCard: hover lift + smooth press (active scale 0.97)", "PackageCard + CuratedPackCard: staggerItemScale grid entrance", "BookingConfirmation: bounceIn checkmark + blur-reveal success card", "MessagesScreen: listSlideIn for conversation threads", "NotificationCenter: listSlideIn + stagger for notification cards", "SearchScreen: overlaySlideUp with blur backdrop", "ProfileScreen: blurIn avatar + staggerItem stat cards + settings rows", "LoyaltyScreen: iOS spring drawer + blur-in stat cards", "SpinWheel: popIn prize reveal with blur trail", "HushhBot: enhanced subtleFloat with brightness glow on hover", "CatalogEditSheet: SPRING_IOS_SHEET drawer with blur(4px) backdrop transition"] },
  { version: "1.39", phase: "Open-Source Enhancement Suite", items: ["canvas-confetti — real confetti explosions on BookingConfirmation success and SpinWheel wins", "fireCelebration() — dual-origin cannon burst with branded purple/gold/green particles", "fireSpinWin() — centered 80-particle burst with gravity for spin wheel prizes", "fireStars() — gold star-shaped confetti for milestones", "@formkit/auto-animate — zero-config list animations on AdminOrders and AdminBookings grids", "Lenis smooth scroll — buttery inertia scrolling across entire app (respects prefers-reduced-motion)", "SpotlightCard — Aceternity-style mouse-tracking spotlight + glowing border on PropertyCard hover", "NumberTicker — Magic UI-style animated counter with spring easing for loyalty points", "TypeWriter — typed.js-style typewriter effect component for rotating text", "Utility files: src/lib/confetti.ts, src/lib/lenis-init.ts", "Reusable components: SpotlightCard, NumberTicker, TypeWriter in src/components/shared/"] },
];
      ...
      <div className="shrink-0 px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid hsl(var(--border) / 0.3)", background: "hsl(var(--background) / 0.95)", backdropFilter: "blur(20px)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted-foreground font-medium">v1.39 · Internal</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
